export default {
  ...
  severity: 'medium',
  conditions: {
    jq: `[select((.displayName as $name | .projects[].iamPolicies[].bindings[].members[] | contains($name) | not)
    or (.displayName as $name | .folders[].iamPolicies[].bindings[].members[] | contains($name) | not))]
    | { "match" : (length > 0) }`,
    path: '@',
    and: [
      {
