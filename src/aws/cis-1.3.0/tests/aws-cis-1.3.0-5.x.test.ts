...
import Aws_CIS_130_53 from '../rules/aws-cis-1.3.0-5.3'
...
describe('CIS Amazon Web Services Foundations: 1.3.0', () => {
  ...
  describe('AWS CIS 5.1 Ensure no Network ACLs allow ingress from 0.0.0.0/0 to remote server administration ports', () => {
    const test51Rule = async (
      ...
      // Act
      const [processedRule] = await rulesEngine.processRule(Aws_CIS_130_51 as Rule, { ...data })
    ...
    test('No Security Issue when there is an inbound rule with security group as source', async () => {
      await test51Rule(undefined, undefined, 'sg-049c76f349f62e4eb', Result.PASS)
    })
  })
  ...
  describe('AWS CIS 5.2 Ensure no security groups allow ingress from 0.0.0.0/0 to remote server administration ports', () => {
    const testRule = async (
      ...
      // Act
      const [processedRule] = await rulesEngine.processRule(Aws_CIS_130_52 as Rule, { ...data })
