...
describe('AWS NIST 800-53: Rev. 4', () => {
  ...
  const testSecurityGroupRule = (
    ...
    test(`No Security Issue when there is an inbound rule with IPv4 wilcard address and a port range not including the port ${fromPort}`, async () => {
      await testRule(
        fromPort + 100,
        fromPort + 200,
        ipV4WildcardAddress,
    ...
    test(`No Security Issue when there is an inbound rule with IPv6 wilcard address and a port range not including the port ${fromPort}`, async () => {
      await testRule(
        fromPort + 100,
        fromPort + 200,
        ipV6WildcardAddress,
    ...
    test(`No Security Issue when there is an inbound rule with IPv6 wilcard address and a port range not including the port ${fromPort} (multiple values)`, async () => {
      await testRule(
        fromPort + 100,
        fromPort + 200,
        ipV6WildcardAddress,
    ...
    test(`Security Issue when IPv4 wilcard address and port ${fromPort}`, async () => {
      await testRule(fromPort, toPort, ipV4WildcardAddress, rule, Result.FAIL)
    })

    test(`Security Issue when IPv6 wilcard address and port ${fromPort}`, async () => {
      await testRule(fromPort, toPort, ipV6WildcardAddress, rule, Result.FAIL)
    })

    test(`Security Issue when IPv4 wilcard address and port ${fromPort} (multiple values)`, async () => {
      await testRule(
        fromPort,
        toPort,
        ipV4WildcardAddress,
    ...
    test(`Security Issue when there is an inbound rule with IPv4 wilcard address and port range includes the port ${fromPort}`, async () => {
      await testRule(0, fromPort + 100, ipV4WildcardAddress, rule, Result.FAIL)
    })

    test(`Security Issue when there is an inbound rule with IPv6 wilcard address and port range includes the port ${fromPort}`, async () => {
  ...
  describe('AWS NIST 8.43 VPC security groups attached to EC2 instances should not permit ingress from ‘0.0.0.0/0’ to all ports', () => {
    const rule = Aws_NIST_800_53_843 as Rule
    ...
    test('No Security Issue when there is an inbound rule with IPv4 wilcard address and a port range not allow all ports', async () => {
      await testRule(1000, 2000, ipV4WildcardAddress, rule, Result.PASS)
    })

    test('No Security Issue when there is an inbound rule with IPv6 wilcard address and a port range not allow all ports', async () => {
      await testRule(1000, 2000, ipV6WildcardAddress, rule, Result.PASS)
    })

    test('Security Issue when IPv4 wilcard address and allow all ports', async () => {
      await testRule(0, 65535, ipV4WildcardAddress, rule, Result.FAIL)
    })

    test('Security Issue when IPv6 wilcard address and allow all ports', async () => {
      await testRule(0, 65535, ipV6WildcardAddress, rule, Result.FAIL)
    })

    test('Security Issue when IPv4 wilcard address and allow all ports (multiple values)', async () => {
    ...
    test('Security Issue when there is an inbound rule with IPv4 wilcard address and port range allow all ports', async () => {
      await testRule(0, 65535, ipV4WildcardAddress, rule, Result.FAIL)
    })

    test('Security Issue when there is an inbound rule with IPv6 wilcard address and port range allow all ports', async () => {
      await testRule(0, 65535, ipV6WildcardAddress, rule, Result.FAIL)
    })
  })
  ...
  describe('AWS NIST 8.45 VPC security groups attached to RDS instances should not permit ingress from ‘0.0.0.0/0’ to all ports', () => {
    const rule = Aws_NIST_800_53_845 as Rule
