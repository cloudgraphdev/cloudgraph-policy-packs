export default {
  id: 'aws-nist-800-53-rev4-8.24',
  title: 'AWS NIST 8.24 VPC security group rules should not permit ingress from ‘0.0.0.0/0’ to TCP/UDP port 2382 (SQL Server Analysis Services browser)',
  ...
  6. Ensure no rule exists that has a port range that includes port *2382* and has a *Source* of *0.0.0.0/0*`,
  ...
  - Remove any rules that include port 2382 and have a source of 0.0.0.0/0.
  - Click Save.
  ...
  **AWS CLI**
  ...
  Remove the inbound rule(s) that permits unrestricted ingress to port 2382:
  
      aws ec2 revoke-security-group-ingress --region <region> --group-name <group_name> --protocol <protocol> --port 2382 --cidr 0.0.0.0/0
  
  Optionally add a more restrictive ingress rule to the selected Security Group:
  
      aws ec2 authorize-security-group-ingress --region <region> --group-name <group_name> --protocol <protocol> --port 2382 --cidr <cidr_block>`,
            ...
            or: [
              ...
              },
              {
                and: [
                  {
                    path: '[*].fromPort',
                    lessThanInclusive: 2382,
                  },
                  {
                    path: '[*].toPort',
                    greaterThanInclusive: 2382,
                  },
                ],
              },
            ],
