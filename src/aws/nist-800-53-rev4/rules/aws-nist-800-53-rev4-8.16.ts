export default {
  id: 'aws-nist-800-53-rev4-8.16',  
  title: 'AWS NIST 8.16 VPC security group rules should not permit ingress from ‘0.0.0.0/0’ to TCP/UDP port 137 (NetBIOS Name Service)',
  ...
  6. Ensure no rule exists that has a port range that includes port *137* and has a *Source* of *0.0.0.0/0*`,
  
  rationale: 'Removing unfettered connectivity to remote console services, such as NetBios Name Service, reduces a server\'s exposure to risk.',
  
  remediation: `**AWS Console**
  
  - Navigate to [VPC](https://console.aws.amazon.com/vpc/).
  - In the left navigation pane, click Security Groups.
  - Remove any rules that include port 137 and have a source of 0.0.0.0/0.
  - Click Save.
  ...
  **AWS CLI**
  ...
  Remove the inbound rule(s) that permits unrestricted ingress to port 137:
  
      aws ec2 revoke-security-group-ingress --region <region> --group-name <group_name> --protocol <protocol> --port 137 --cidr 0.0.0.0/0
  
  Optionally add a more restrictive ingress rule to the selected Security Group:
  
      aws ec2 authorize-security-group-ingress --region <region> --group-name <group_name> --protocol <protocol> --port 137 --cidr <cidr_block>`,
            ...
            or: [
              ...
              },
              {
                and: [
                  {
                    path: '[*].fromPort',
                    lessThanInclusive: 137,
                  },
                  {
                    path: '[*].toPort',
                    greaterThanInclusive: 137,
                  },
                ],
              },
            ],
