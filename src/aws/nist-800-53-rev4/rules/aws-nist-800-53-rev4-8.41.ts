export default {
  id: 'aws-nist-800-53-rev4-8.41',
  title: 'AWS NIST 8.41 VPC security group rules should not permit ingress from ‘0.0.0.0/0’ to TCP port 9200 (Elasticsearch)',
  
  description: 'VPC security groups should not permit unrestricted access from the internet to port 9200 (Elasticsearch). Removing unfettered connectivity to an Elasticsearch server reduces the chance of exposing critical data.',
  ...
  6. Ensure no rule exists that has a port range that includes port *9200* and has a *Source* of *0.0.0.0/0*`,
  ...
  **AWS CLI**
  ...
  Remove the inbound rule(s) that permits unrestricted ingress to port 9200:
      ...
      ws ec2 revoke-security-group-ingress --region <region> --group-name <group_name> --protocol <protocol> --port 9200 --cidr 0.0.0.0/0
            ...
            or: [
              ...
              },
              {
                and: [
                  {
                    path: '[*].fromPort',
                    lessThanInclusive: 9200,
                  },
                  {
                    path: '[*].toPort',
                    greaterThanInclusive: 9200,
                  },
                ],
              },
            ],
